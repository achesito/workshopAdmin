...
All these values ​​are example data. In this last case, to access this functionality, you will need to create an account on the Brevo platform. Register with your business email or any email you prefer and create an API key. Go to settings, then to SMTP and API, select the "API and MCP keys" option, and generate your own API key. Then, replace the value above "your api key" in that variable with the one generated by the platform.

Don't forget to replace the blank space with "_"
...
EMAIL_KEY = "your api key"
todos estos valores son datos ejemplos, en este ultimo caso, para acceder a esta funcionalidad necesitaras crear una cuenta en la plataforma llamada brevo, registrarte con el respectivo correo del negocio o con el que prefieras y crear una api-key, entrando a configuracion, luego a smtp y api, seleccionar la opcion "claves api y mcp" y generar tu propia api, luego remplazas el valor de arriba de "your api key" en esa variable, por el que te genero la plataforma.

No olvides reemplazar el espacio en blanco por "_"